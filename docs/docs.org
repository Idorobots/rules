...
Implementacja ta została porównana z /naiwną/ implementacją, która dla każdej z =R= reguł kombinatorycznie unifikuje =F= faktów w =P= wzorcach, skutkując złożonością obliczeniową rzędu =O(RF^P)=.
